...
public:
    sparse_set()
      : dense_arr(), sparse_arr(INIT_CAPACITY) {}
    ~sparse_set() = default;
...
public:
    ...
    auto erase(const value_type &value) -> size_type {
        ...
        sparse_arr[hash_value(dense_arr.back())] = p;
        dense_arr[p]                             = std::move(dense_arr.back());
        dense_arr.pop_back();
    ...
    auto contains(const value_type &value) const -> bool {
        size_type hashed = hash_value(value);
        size_type p      = sparse_arr[hashed];
        return p < size() && key_equal{}(dense_arr[p], value);
    }
...
private:
    ...
    std::vector<
        size_type,
        typename std::allocator_traits<allocator_type>::template rebind_alloc<size_type>>
        sparse_arr;
...
private:
    // FIXME: bias hashing
    auto hash_value(const value_type &value) const -> size_type {
